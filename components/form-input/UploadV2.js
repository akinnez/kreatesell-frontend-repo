...
export const DropzoneV2 = ({
	...
	return (
		<>
			<div
				className={`${
					variant === 2 ? 'customUploadV2' : 'customUploadV3'
				} ${style.dragger_wrapperV2} ${
					variant === 3 && style.dragger_wrapperV3
				} }`}
			>
				<label className={style.label}>
					<span style={{color: 'red'}}>{extraLabel}</span> {label}{' '}
				</label>
				<AntUpload.Dragger
					{...rest}
				...
				>
					{!!accept && imgUrl ? (
						<div className={style.otheFileDescription}>
							<h1 className={style.fileName}>{accept} file:</h1>{' '}
							{fileName}
						</div>
					) : imgUrl || value ? (
						<Image
			...
			{`${name}` in errors && errors[name] && (
				<p className={style.error} style={{display: 'block'}}>
					{name} is required
				</p>
			)}
		</>
