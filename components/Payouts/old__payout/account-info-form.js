...
import {Button, Input, Select} from 'components/form-input';
...
const BankModal = ({open, onClose = () => {}}) => {
	...
	useEffect(() => {
		ApiService.request(
			...
			({data}) => {
				const countries = data?.list_of_countries?.map(
					({id, name}) => ({
						label: name,
						value: id,
					})
		...
		<>
			...
			>
				...
				<p className={style.sub_title}>
					We pay your funds to this account
				</p>
				<Form layout="vertical" form={form} onFinish={handleSubmit}>
					<Select
						...
						name="country_id"
						rules={[
							{required: true, message: 'Country is important'},
						]}
					/>
					...
					<Input
						...
						rules={[
							{
								required: true,
								message: 'Account Number is important',
							},
						]}
					/>
					...
					<Input
						...
						rules={[
							{
								required: true,
								message: 'Account Name is important',
							},
						]}
					/>
					...
					<div className={style.alert}>
						<p>
							...
							proceeding. We will not be held liable for failed
							transactions resulting from incorrect bank details.
						</p>
					</div>
					<Input
						size="large"
						...
						name="password"
						rules={[
							{required: true, message: 'Password is important'},
						]}
					/>
