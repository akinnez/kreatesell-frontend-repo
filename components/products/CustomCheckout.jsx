...
export default function CustomCheckoutSelect({title, field, setField}){
  ...
  const getAllowedCurrencies = GetAllowedCurrencies();
    ...
    const filterCurrency = useMemo(()=> currency?.map(cur=> cur.short_name), [currency]);
    const countriesCurrency = useMemo(()=> countries?.filter(country=> country.currency_id !== null && filterCurrency?.includes(country.currency)), [countries, filterCurrency])
    const addCurrency = ()=>{
    ...
    return(
        <div className="">
            ...
            <h2 className={`${styles.currencyHeader} mt-3 mb-3`}>Selected Currencies</h2>
