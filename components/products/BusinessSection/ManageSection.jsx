    ...
    return(
        <div className="">
            ...
            <div className="flex items-center justify-between mb-7">
                <h1 className={`text-2xl text-blue-600 font-bold ${styles.titleMain}`}>How to Invest in Crypocurrency</h1>
               
                <div className={styles.miniSaveButtons + " flex"}>
                    <Button type="primary" style={{color: "#0072ef"}}>+ Add Section</Button>
                    <Button type="primary" icon={<Image color="white" src={WhiteEye} alt="empty"/>}>  &nbsp;Preview Membership</Button>
                </div>
            </div>
...

            {
                productSection !== null && productSection.length > 0 && productSection.map((items, index)=>(
                    <ManageSectionSegment
                        key={index}
                        {...{items,index, setProductSection, handleChange, formik, setFieldValue, openMedia, deleteLecture, toSection}}
                    />
