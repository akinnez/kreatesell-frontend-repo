...
const ManageSection = ({items,index, setProductSection, handleChange, formik, setFieldValue, openMedia, deleteLecture, toSection}) => {
                        ...
                        <div className="flex justify-between items-center">
                            <div className="flex items-center">
                              ...
                              <Input 
                                ...
                                style={{width:"9rem"}}
                              />:
                              <h1 className={`text-2xl font-semibold mb-0 ${styles.titleMain2}`}>{items.section_name}</h1>}
                                
                                <button className='ml-3' onClick={()=>setIsTitleEditable(prev=> !prev)}>Edit</button>
                            </div>
                            ...
                            items?.is_access_control_set && <div className="mb-5">
                                ...
                                <div className="mt-3">
                                    <Radio.Group className={styles.rad}  onChange={(e)=>handleChange(items, e)} defaultValue={true}>
                                        <Radio className={`text-xl font-semibold items-center ${styles.accessControl}`} value={true}>Available to all Subscribers</Radio>
                                        <Radio className={`text-xl font-semibold items-center ${styles.accessControl}`} value={false}>Only available to subscribers who has made payment up to:</Radio>
                                    </Radio.Group>
                                </div>
                                ...
                                <div key={indx} className={styles.managedContent + " bg-white flex justify-between mt-5 items-center rounded-lg mb-1"}>
                                    <div className="flex items-center">
                                        ...
                                        <div className="flex flex-col">
                                            <h1 className={`text-xl font-semibold ${styles.productName}`}>{item.product_section_name}</h1>
                                            {<h2 className={`text-base font-medium ${styles.digitalProductSize}`}>{`20MB`}</h2>}
                                        </div>
                                    </div>
