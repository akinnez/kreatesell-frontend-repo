...
export default function PreviewContent({
	...
	const getAffliatekeys = async () => {
		if(!ref) return
		const response = await axios.get(`${process.env.BASE_URL}/affiliate/confirm-short-link/${ref}`)
	...
	const getCheckoutLink = () => {
		if (affiliateRef && uniqueKey && monthDifference <= 6) {
			return router.push(
				`/checkout/${productId}?${uniqueKey &&
				`affiliateUniqueKey=${uniqueKey}`
				...
				<div className={styles.description}>
					...
					<div className={styles.priceSection}>
                      ...
                      `}
									...
									{productPriceType ===
										'Pay What You Want' && (
											<h1 className="text-xl md:text-3xl font-bold">{`${alreadyDefinedPrice?.currency_name ||
												convertedCurrency?.to_currency_name ||
												getMinimumCurrency()
												} 
