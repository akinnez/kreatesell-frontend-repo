...
const BlogTab = ({ active, setActive }) => {
  ...
  const sortCategories = () => {
    if(categories){
      categories?.forEach((cat)=> setCategoriesData(prev=>[...prev, {value: cat?.category}]));
    }
  }
        ...
        <div key={`${tab.value} ${i}`} className={styles.tabInner}>
          <button
            ...
            style={{
              ...
              background: active === tab.value ? "#0072ef" : "transparent",
              border: active === tab.value ? "none" : "1px solid #8c8c8c",
