...
const Coupon = () => {
	...
	const { loading, coupons } = useSelector(
        (state) => state.coupon
      );
	useEffect(()=>{
		getCoupon()
	...
	const memoisedCouponData = useMemo(
		() =>
			couponData
				?.sort((a, b) =>
					a.coupons?.date_created < b.coupons?.date_created
						? 1
						: -1
				)
				?.map((item, i) => ({
					...
					key: i+1,
					numbers: i+1,
					product_name: item.coupons.is_for_all_product ? 'All Products': 'Item',
					...
					usages: item?.usage,
					discount: item.coupons.is_percentage ? `${item.coupons.percentage_value}%` : `NGN ${item.coupons.fixed_amount_value}`,
					max_usages: item.coupons.number_of_usages === -1 ? 'N/A' : item.coupons.number_of_usages,
					quantity: 1,
					more: {
						id: item.coupons.id
					} 
