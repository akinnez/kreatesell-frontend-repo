...
const AccessPageModal = ({
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const fileTobeDownloaded = response?.data?.product_dto?.product_images?.filter((item) => item?.file_type_name === 'ContentZipFile')
			handleDownload(
				fileTobeDownloaded[0]?.filename
			);
		} catch (error) {
			setErrorModal(true);
		} finally {
...
const PurchaseSummaryCard = ({ handleClickAction, productName, product }) => {
	...
	const contentZipFile = product?.product_images?.filter((item) => item.file_type_name === "ContentZipFile");
	...
	return (
		<div className={styles.purchaseSummaryCardContainer}>
			...
			{contentZipFile.length > 0 && product?.product_type_details === 'Digital Download' ? (
				...
				<div className={styles.noContentPurchase}>
					<Image
						...
						alt=""
					/>
					<div className={styles.noContentText}>Content File is Unavailable</div>
				</div>
			)}
			<br />
			<hr />
			...
			{contentZipFile?.length === 0 && productType === 'Digital Download' && (
