...
const Checkout = () => {
	...
	const baseCurrencyObbject = checkOutDetails.find((item) => item.currency_name === defaultCurrency)

	const checkout = checkOutDetails?.filter(
		// (item) => item?.currency_name === activeCurrency?.currency,
		(item) => item?.price_indicator === pricingTypeDetails.price_type === 'Pay What You Want' ? 'Minimum' : 'Selling' && item?.currency_name === baseCurrencyObbject.currency_name
	);
	...
	const paymentDetails = ({ reference = '', status = '' }) => {
		...
		const value = {
			...
			is_free_flow: pricingTypeDetails.price_type === 'Make it Free' ? true : false,
		};
		return value;
	};
                      ...
                      <s className="text-base-gray-200">
                        {currency_name} 10000
												...
												<p>
													...
													{OriginalPrices && (
														<span style={{ fontSize: '15px', color: '#8C8C8C', textDecoration: 'line-through' }}>{getCurrency('currency')}{' '}</span>
													)}
													<span style={{ fontSize: '15px', color: '#8C8C8C', textDecoration: 'line-through', marginRight: '7px' }}>{getCurrency('original')}</span>
													{getCurrency('currency')}{' '}
													{subTotal}
										...
										<div className="flex justify-between">
                                            ...
                                            price ?? checkoutDetails?.default_price
												...
												{getCurrency('currency')} {Number(totalFee).toFixed(2)}
											</p>
										</div>
									</div>
								)}
