...
const Checkout = () => {
	...
	const getCurrency = (priceOrName) => {
		if (priceOrName === 'currency') {
			...
			return taxValue ? getTaxDeduction('getCal') : totalFee + transactionFee;
	...
	useEffect(() => {
		...
		getTransactionFees();
	}, [activeCurrency?.currency, activeCurrency?.currency_name, subTotal, isChargable]);
                        ...
                        {currency_name} 10000
										...
										<div className="flex justify-between">
											...
											<p className="text-primary-blue font-medium">
												{getCurrency('currency')}{' '}
												{Number(getCurrency('total')).toFixed(2)}
											</p>
										</div>
									</div>
								)}
