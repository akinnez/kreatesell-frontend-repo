...
const Checkout = () => {
	...
	const getTaxDeduction = (val) => {
		...
		return val === 'getVal' ? calTax : Number(calTax) + Number(subTotal) + Number(transactionFee);
	};
	...
	useEffect(() => {
		const getTransactionFees = async () => {
			...
			try {
				await axios.get(`https://kreatesell.io/api/v1/kreatesell/product/get-bearable-fee/${activeCurrency?.currency_name || activeCurrency?.currency}`)
					//get actual fees and percentage indications 
					.then((res) => {
						const value = res?.data?.data?.fee_amount
						const percentageValue = Number(((value / 100) * subTotal).toFixed(2))
                        ...
                        {currency_name} 10000
										...
										<div className="flex justify-between">
											...
											<p className="text-primary-blue font-medium">
												{getCurrency('currency')}{' '}
												{taxValue
													? Number(getTaxDeduction('getCal')).toFixed(2)
													: Number(totalFee + transactionFee).toFixed(2)
												}
											</p>
										</div>
									</div>
